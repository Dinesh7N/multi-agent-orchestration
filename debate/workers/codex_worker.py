...
class CodexWorker(RedisWorker):
    async def process(self, payload: dict[str, str]) -> None:
        if payload.get("agent") != AgentType.CODEX.value:
            return
        task_slug = payload["task_slug"]
        round_number = int(payload["round"])
        phase = Phase(payload.get("phase", Phase.IMPLEMENTATION.value))
        ...
        if role_value:
            try:
                ...
                await run_agent(task_slug, AgentType.CODEX, round_number=round_number, phase=phase)
                return
...
def main() -> None:
    worker = CodexWorker(agent=AgentType.CODEX.value, group="codex-workers")
